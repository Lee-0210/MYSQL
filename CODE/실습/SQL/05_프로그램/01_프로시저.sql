...
CALL insert_new_employee('조은', '김', STR_TO_DATE('2000-02-02', '%Y-%m-%d'), 'M', 'd005', 3300000);

-- OUT 파라미터 활용
-- emp_no 사원 번호를 입력하여 IN 파라미터로 입력하고,
-- 해당 사원의 최근 발령일자와 급여를 반환하는 프로시저를 정의하시오.
-- OUT : 발령일자, 급여

SELECT MAX(from_date) AS '최근 발령일'
FROM dept_emp
WHERE emp_no = 500000
GROUP BY emp_no
ORDER BY `최근 발령일` DESC;
...
DROP PROCEDURE get_last_date_and_salary;
...
BEGIN
    -- 최근 발령일자 조회
    SELECT MAX(from_date) AS '최근 발령일' INTO p_last_date
    FROM dept_emp
...
SELECT @last_date AS '최근 발령일자', @last_salary AS '최근 급여';
