-- 사원의 월급 계산 함수
DROP FUNCTION calculate_month_salary;
DELIMITER //
CREATE FUNCTION calculate_month_salary(p_emp_no INT)
RETURNS DECIMAL(10,2)
DETERMINISTIC
READS SQL DATA
BEGIN
    DECLARE month_salary DECIMAL(10,2);

    SELECT salary / 12
    INTO month_salary
    FROM salaries
    WHERE emp_no = p_emp_no
    AND to_date = '9999-01-01'
    ORDER BY from_date DESC
    LIMIT 1;

    RETURN month_salary;
END //
DELIMITER ;

SELECT emp_no, first_name, gender, calculate_month_salary(emp_no) AS '월급'
FROM employees;

DROP FUNCTION calculate_years_of_service;
-- 사원의 근무 기간(년) 계산 함수
DELIMITER //
CREATE FUNCTION calculate_years_of_service(p_emp_no INT)
RETURNS INT
DETERMINISTIC
READS SQL DATA
BEGIN
    DECLARE years INT;

    SELECT TIMESTAMPDIFF(YEAR, hire_date, CURDATE())
    INTO years
    FROM employees
    WHERE emp_no = p_emp_no;

    RETURN years;
END //
DELIMITER ;

DROP FUNCTION get_department_name;
DELIMITER //
CREATE FUNCTION get_department_name(p_dept_no CHAR(4))
RETURNS VARCHAR(40)
DETERMINISTIC
READS SQL DATA
BEGIN
    DECLARE v_dept_name VARCHAR(40);

    SELECT dept_name
    INTO v_dept_name
    FROM departments
    WHERE dept_no = p_dept_no;

    RETURN v_dept_name;
END //
DELIMITER ;

SELECT e.emp_no, first_name, gender
      ,calculate_month_salary(e.emp_no) AS '월급'
      ,calculate_years_of_service(e.emp_no) AS '근속년수'
      ,get_department_name(d.dept_no) AS '부서이름'
FROM employees e INNER JOIN dept_emp d
ON e.emp_no = d.emp_no;